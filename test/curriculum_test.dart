...
void main() {
  group('Alphabet and Orthography Data', () {
    test('igboLetters36 has exactly 36 entries and contains special characters and digraphs', () {
      ...
      expect(igboLetters36.toSet().length, 36, reason: 'All letters should be unique');
      expect(igboLetters36, containsAll(['ị', 'ọ', 'ụ', 'ṅ']));

      for (final digraph in igboDigraphs) {
        expect(igboLetters36, contains(digraph), reason: 'Digraph $digraph should be in igboLetters36');
      }
    });
  ...
  group('Curriculum Structure and Units', () {
    test('curriculum has exactly 9 units with ids 1 through 9 in sequential order', () {
      expect(curriculum.length, 9);
      for (var i = 0; i < curriculum.length; i++) {
        expect(curriculum[i].id, i + 1);
      }
    });

    test('unitById returns the corresponding unit and throws for invalid id', () {
      for (var i = 1; i <= 9; i++) {
        final unit = unitById(i);
        expect(unit.id, i);
      }
      expect(() => unitById(0), throwsArgumentError);
      expect(() => unitById(10), throwsArgumentError);
    });

    test('every unit has 4 to 6 lessons and valid titles', () {
      for (final unit in curriculum) {
        expect(unit.titleIgbo.trim(), isNotEmpty, reason: 'Unit ${unit.id} titleIgbo must not be empty');
        expect(unit.titleEn.trim(), isNotEmpty, reason: 'Unit ${unit.id} titleEn must not be empty');
        expect(unit.lessons.length, inInclusiveRange(4, 6),
      ...
      for (final unit in curriculum) {
        for (final lesson in unit.lessons) {
          expect(lesson.title.trim(), isNotEmpty,
              reason: 'Lesson ${lesson.id} in Unit ${unit.id} title must not be empty');
          expect(lesson.questions.length, inInclusiveRange(4, 8),
              reason: 'Lesson ${lesson.id} in Unit ${unit.id} should have between 4 and 8 questions');
        }
      }
    });
      ...
      for (final unit in curriculum) {
        for (final lesson in unit.lessons) {
          for (final question in lesson.questions) {
            expect(idPattern.hasMatch(question.id), isTrue,
                reason: 'Question ID ${question.id} must match format u<unit>l<lesson>q<question>');
            expect(questionIds.contains(question.id), isFalse,
  ...
  group('Question Quality and Invariants', () {
        ...
        for (final lesson in unit.lessons) {
          for (final question in lesson.questions) {
            if (question.type == QuestionType.matchPairs) {
              expect(question.pairs.length, inInclusiveRange(3, 5),
              ...
              for (final pair in question.pairs) {
                expect(pair.left.trim(), isNotEmpty,
                    reason: 'MatchPairs ${question.id} pair left side must not be empty');
                expect(pair.right.trim(), isNotEmpty,
                    reason: 'MatchPairs ${question.id} pair right side must not be empty');
                expect(rightValues.contains(pair.right), isFalse,
                    reason: 'MatchPairs ${question.id} right value "${pair.right}" must be unique');
        ...
        for (final lesson in unit.lessons) {
          for (final question in lesson.questions) {
            if (question.type == QuestionType.fillBlank) {
              expect(question.prompt, contains('___'),
                  reason: 'FillBlank ${question.id} prompt must contain "___"');
              expect(question.wordBank.length, inInclusiveRange(3, 4),
                  reason: 'FillBlank ${question.id} wordBank must have 3 to 4 words');
              expect(question.wordBank.toSet().length, question.wordBank.length,
                  reason: 'FillBlank ${question.id} wordBank must not contain duplicates');
              expect(question.answer, isNotNull,
                  reason: 'FillBlank ${question.id} answer must not be null');
              expect(question.wordBank, contains(question.answer),
                  reason: 'FillBlank ${question.id} wordBank must contain the answer');
            }
          }
        }
      }
          ...
          for (final question in lesson.questions) {
            if (question.type == QuestionType.translate) {
              ...
              expect(question.acceptedAnswers, isNotEmpty,
                  reason: 'Translate ${question.id} acceptedAnswers must not be empty');
              expect(question.acceptedAnswers, contains(question.answer),
                  reason: 'Translate ${question.id} acceptedAnswers must contain answer');

              // Check if English->Igbo minimal pair requires toneLenient == false
              final isEnglishToIgbo = question.prompt.toLowerCase().contains('igbo for');
              if (isEnglishToIgbo &&
                  tonalMinimalPairWords.any((w) => question.answer!.contains(w))) {
                expect(question.toneLenient, isFalse,
                    reason: 'Translate ${question.id} with minimal pair answer must have toneLenient false');
              }
            }
          }
        }
    ...
    test('Unit 1 uses only mcq, matchPairs, fillBlank (no translate)', () {
      for (final lesson in unit01.lessons) {
        for (final question in lesson.questions) {
          expect(question.type, isNot(QuestionType.translate),
              reason: 'Unit 1 question ${question.id} must not be translate');
        }
      }
    });

    test('Units 3 to 9 contain at least one translate question', () {
      for (var id = 3; id <= 9; id++) {
        final unit = unitById(id);
        final hasTranslate = unit.lessons.any((lesson) =>
            lesson.questions.any((q) => q.type == QuestionType.translate));
        expect(hasTranslate, isTrue, reason: 'Unit $id must contain translate questions');
      }
    });
  });
  ...
  group('Vocabulary and Orthography Integrity', () {
    ...
    test('spot checks for specific vocab and lesson contents', () {
      expect(unit03.vocab.any((v) => v.igbo == 'ndewo'), isTrue,
          reason: 'Unit 3 vocab must contain ndewo');
      expect(unit04.vocab.any((v) => v.igbo == 'otu'), isTrue,
          reason: 'Unit 4 vocab must contain otu');
      expect(unit04.vocab.any((v) => v.igbo == 'iri'), isTrue,
          reason: 'Unit 4 vocab must contain iri');

      final unit1HasGb = unit01.lessons.any((lesson) => lesson.questions.any((q) =>
          q.prompt.contains('gb') ||
          q.options.contains('gb') ||
          q.answer == 'gb' ||
          q.pairs.any((p) => p.left == 'gb' || p.right == 'gb')));
      expect(unit1HasGb, isTrue, reason: 'Unit 1 must contain a question referencing the gb digraph');
    });

    test('NO em dashes (U+2014) or en dashes (U+2013) anywhere in content strings', () {
      const emDash = '\u2014';
      const enDash = '\u2013';

      void assertNoDash(String str, String location) {
        expect(str.contains(emDash), isFalse,
            reason: 'Found em dash in $location: "$str"');
        expect(str.contains(enDash), isFalse,
